...
def find_spin_gap(results,envelope_model,bounded_state_tolerance=None,positional_max_tv_dist=1e-4,find_optimal=True):
    """
    ...
    for res_i,result in enumerate(results):
        ...
        if ground_state_i is None:
            ...
            if find_optimal:
                closest = np.argmin(log_overlaps)+ground_state_i+1 # +gs_i+1 because we do not include the gs in the log_overlaps os if gs_i= 0 and argmin=0 the exited state should be 1
        
                LOGGER.info(f'picking closest exited state {closest} with infidelity {log_overlaps[closest-ground_state_i-1]}')
                spin_gaps.append(eigen_vals[closest]-eigen_vals[ground_state_i])
                wave_functions.append((eigen_vects[ground_state_i],eigen_vects[closest]))
                has_intermediate_state.append(bool(closest-ground_state_i -1))
            else:
                spin_gaps.append(np.nan)
                wave_functions.append((None, None))
                has_intermediate_state.append(None)
